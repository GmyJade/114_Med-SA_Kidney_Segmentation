    ...
    if c >= 1: 
        if points is not None:
            if point_labels is not None:
                point_labels = torch.as_tensor(point_labels, device=points.device)
                if point_labels.dim() == 0:
                    point_labels = point_labels.reshape(1, 1).repeat(b, 1)
                elif point_labels.dim() == 1:
                    if point_labels.numel() == b:
                        point_labels = point_labels.reshape(b, 1)
                    else:
                        point_labels = point_labels.reshape(1, -1).repeat(b, 1)
                else:
                    point_labels = point_labels.reshape(point_labels.shape[0], -1)
                    if point_labels.shape[0] != b and point_labels.numel() == b:
                        point_labels = point_labels.reshape(b, 1)
            ...
            for i in range(b):
                ...
                for k in range(len(py)):
                    ...
                    # Color logic
                    label_idx = min(k, lbls.numel() - 1)
                    if lbls[label_idx].item() == 1:
